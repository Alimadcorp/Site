...
async function onloaded() {
  ...
  // trigger an initial load of online users, then call the function 10 seconds after successfully getting the count previously :sob:
  (
    function live() {
      fetch("https://live.alimad.co/ping?app=alimad.co:online").then(r => r.text()).then(d => onLive(d));
      if (!window.lives) {
        setInterval(live, 10000); // start interval if it doesnt already exist
        window.lives = true;
      }
    }
  )();

  const getVisits = () => fetch("https://live.alimad.co/stats?app=alimad.co").then(r => r.json()).then(d => onLive(d));

  if (shouldLog()) fetch("https://live.alimad.co/ping?app=alimad.co").then(_ => getVisits());
  else getVisits();

  // we have 2 keys, one to count visits, one to count currently online users, as if we fetch the key every 10 seconds, it would count a "visit" every 10 seconds :sob:
  // also, we reset from alimadhomepage to alimad.co
...
function shouldLog() {
  const key = "live.alimad.co:lastLog";
